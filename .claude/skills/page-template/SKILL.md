...
- sim ที่วาดลง canvas ต้อง resolve สีจาก CSS token และฟัง event `sky-physic:themechange` เพื่อ re-resolve (ดู `shared/sim/vector3d.js` เป็นแบบ)
- **skin ต้องการฟอนต์ภายนอก:** ห้าม `@import` ตรงในไฟล์ `shared/skins/<name>.css` เด็ดขาด — `@import` ไม่ถูก scope
  ด้วย `[data-skin]` จะโหลดทุกหน้าไม่ว่า skin ไหน active อยู่ (เจอบั๊กนี้จริงตอนทำ comic skin, ดู
  `docs/superpowers/plans/2026-07-04-theme-system.md` ส่วน "Post-review fix") ให้ประกาศ `fontsUrl` ในทะเบียน skin
  ของ `shared/config.js` แทน แล้วให้ `shared/layout.js` แทรก `<link rel="stylesheet">` เองเฉพาะตอน skin นั้น active
